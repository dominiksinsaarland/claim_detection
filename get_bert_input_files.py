...
if __name__ == "__main__":
	...
	with open(os.path.join(path_outfiles, "CLEF_train_input.txt"), "w") as outfile:
	...
	train.extend(utils.ClaimBuster_Loader().get_examples(block_set=block_set))
	with open(os.path.join(path_outfiles, "CLEF_AND_CLAIMBUSTER_train_input.txt"), "w") as outfile:
		for sentence, label in train:
			outfile.write(sentence + "\t" + str(label) + "\n")
